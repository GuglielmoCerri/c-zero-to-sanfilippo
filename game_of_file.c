...
/* This function sets the specified cell at x,y to the specific state */
void set_cell(char *grid, int x, int y, char state) {
    grid[cell_to_index(x,y)] = state;
}

/* This function gets the state of the grid at x, y */
char get_cell(char *grid, int x, int y) {
    return grid[cell_to_index(x,y)];
}

/* Print the grid on the screen, clearing the screen
 * before doing it. */
...
/* Return the number of living neighbors for x, y. */
...
/* Compute the new state of Game of Life. */
